...
curl -X PUT -d "[$e_time]@[ERROR]: $error_code" http://${local_ip}:${port}/v1/kv/cmha/service/$servicename/alerts/alerts_counter/$alerts_id
}

monitor_kv_alerts(){
	for Node in `curl -s -X GET http://${local_ip}:${port}/v1/kv/cmha/service/$servicename/db/repl_err_counter?keys|jq '.'|awk -F'"' '{print $2}'|sed '/^$/d'|awk -F'/' '{print $6}'`
	do
		rs=`curl -s -X GET http://${local_ip}:${port}/v1/kv/cmha/service/$servicename/db/repl_err_counter/${Node}?raw`
		Address=`curl -s -X GET http://${local_ip}:${port}/v1/health/service/${servicename}|jq -c ".[].Node|select(.Node==\"$Node\")"|jq -r .Address`
				...
				if [ $(grep "$c_text" $base_dir/.${Node}_alerts_counter) ];then
                        		...
                        		local S_time=$(($C_time-$L_time))
                                	if [ $S_time -gt $frequency ];then
                                		put_alert_kv
                                		echo "${e_time} ${error_code}" >>$log_file                        
...
service_alerts(){
             if [ "$(echo $Output|grep -w 'haproxy')" ];then
             		...
             		error_code="[$servicename] $Node $Address replication IO_thread $Status"
             		put_alert_kv
             		echo "${e_time} ${error_code}" >>$log_file                     
...
case $1 in
	service)
curl -s -X GET http://${local_ip}:${port}/v1/health/service/${servicename}|jq -c '.[].Checks[]|{Status,CheckID,Node,Output}'|jq -c 'select(.Status=="critical" or .Status=="warning")' >$base_dir/.$$_node_alerts
	;;
	CS)
	touch $base_dir/.$$_node_alerts
	for i in `awk -F= '/service_ip/ {print $2}' $config_file|sed 's/[ ]//g'|awk -F';' '{print $1,$2,$3}'` 
	do
	node=`curl -s -X GET http://${local_ip}:${port}/v1/catalog/nodes|jq -c ".[]|select(.Address==\"$i\")"|jq -r .Node`
curl -s -X GET http://${local_ip}:${port}/v1/health/state/critical|jq -c '.[]|{Status,CheckID,Node,Output}'|jq -c "select(.Node==\"$node\")" >>$base_dir/.$$_node_alerts
	done
	;;
esac
...
if [ ! -z $base_dir/.$$_node_alerts ];then
	...
	do
		...
		CheckID=`echo $LINE|jq -r '.CheckID'`
		Node=`echo $LINE|jq -r '.Node'`
		Output=`echo $LINE|jq -r '.Output'`
                Address=`curl -s -X GET http://${local_ip}:${port}/v1/catalog/nodes|jq -c ".[]|select(.Node==\"$Node\")"|jq -r .Address`	
		if [ -e $base_dir/.${Node}_alerts_counter ];then
...
sleep 5
cs_leader=`curl -s -X GET http://${local_ip}:${port}/v1/status/leader|jq -r '.'|awk -F: '{print $1}'`
