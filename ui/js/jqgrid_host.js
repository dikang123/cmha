var pagefunction = function() {
    ...
    var getHostName = function() {
        ...
        var array_ServiceName = {};
        var getServiceName = function() {
            $.ajax({
                method:"get",
                url:"http://" + IP + "/v1/catalog/node/" + hostName,
                async:false,
                dataType:"json",
                success:function(result, status, xhr) {
                    array_ServiceName = {};
                    array_ServiceName = result;
                },
                error:function(XMLHttpRequest, status, jqXHR, textStatus, e) {
                    console.error("失败状态文本 " + status);
                }
            });
            var bbbb = array_ServiceName.Services;
            var bbbbb = [];
            for (var key in bbbb) {
                bbbbb.push(key);
            }
    ...
    var run_jqgrid_switch_function = function() {
        ...
        var changeData_SHV = function() {
            ...
            var after_decodeData_SHV_monitor_a = function(obj_valueOfSHV, obi_variableArray, obj_time) {
                ...
                console.timeEnd("after_decodeData_SHV_monitor_v  运行时间--");
            };
